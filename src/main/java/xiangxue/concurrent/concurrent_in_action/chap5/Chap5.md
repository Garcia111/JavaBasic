...
【**信号量的初始值设定**】
    信号量的数据结构为一个值和一个指针，**指针**指向等待该信号量的下一个进程，**信号量的值**与相应资源的使用情况有关。
    除了初始赋值外，信号量仅能通过PV操作来访问。

    信号量值S>=0 :表示当前可用资源的数量
           S<0  :其绝对值表示等待使用该资源的进程个数

    信号量按其用途可以分为两种：
    1.公用信号量：
        用于联系一组并发进程，相关的进程均可在此信号量上执行PV操作，初始值常常为1，用于实现进程互斥；
    2.私有信号量
        仅允许拥有此信号量的进程执行P操作，其他相关进程可以在其上执行V操作，初始值常常为0或正整数，用于实现进程同步。

    假定S表示信号量，将P操作和V操作记为P(S)和V(S),由于P(S)和V(S)都是在同一个信号量S上操作，为了使他们在执行时
    不发生因交叉访问信号量S而可能出现的错误，约定P(S)和V(S)必须是两个不可被中断的过程，不可被中断的过程也被称作原语。

    P操作的动作是：
    1.信号量S减1；
    2.若S减1之后仍然大于或等于0，则进程继续执行；
    3.如果S减1之后小于0，则该进程被阻塞之后放入等待该信号量的等待对列中，然后转向进程调度

    V操作的主要动作是：
    1.S加1；
    2.若加1后结果大于0，则进程继续执行；
    3.若加1后结果小于或等于0，则从该信号的等待队列中释放一个等待进程，然后再返回原进程继续执行或转进程调度。
